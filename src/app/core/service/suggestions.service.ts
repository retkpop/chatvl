...
export class SuggestionsService {
  ...
  updateAddSuggestionsUsingByGet(dataContext: DataContext) {
    ...
    // tslint:disable-next-line:max-line-length
    return this.http.post<SuggestionsDTO>(`${environment.apiUrl}/api/suggestions/new`, dataContext, {
      withCredentials: true
    });
  }
  getSuggestions(page: number, limit: number) {
    // tslint:disable-next-line:max-line-length
    return this.http.get<SuggestionsDTO>(
      `${environment.apiUrl}/api/suggestions/get-suggestions-custom/${page}/${limit}`,
      {
        withCredentials: true
      }
    );
  ...
  getSuggestionBySlugUsingByGet(slug: string) {
    // tslint:disable-next-line:max-line-length
    return this.http.get<SuggestionsDTO>(`${environment.apiUrl}/api/suggestions/get-suggestion-by-slug/${slug}`, {
      withCredentials: true
    });
  }
}
