...
export class VideosComponent implements OnInit, OnDestroy {
  ...
  getSubscribeUserPost(subscribeId: number) {
    this.userService
      .getSubscribeUserPost(subscribeId)
      .subscribe(
        datas => {
          if(datas.code === 2001) {
  ...
  getVideoUsingByGet() {
    ...
    return this.postService
      .getVideoBySlugUsingByGet(this.slug)
      .subscribe(posts => {
  ...
  updateViewed(id:number) {
    ...
    if(viewed) {
      ...
      if(index === -1) {
        this.postService.updateViewPost(id).pipe().subscribe();
      }
    } else {
  ...
  getPostByIdCat(postVd: PostResponse) {
    return this.postService
      .getPostByIdCat(postVd.categories.id, 0, 20)
      .subscribe(postsCat => {
